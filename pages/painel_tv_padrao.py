# -*- coding: utf-8 -*-
"""
PAINEL TV · PERÍCIAS & AUDIÊNCIAS — Núcleo Manacapuru (V360 Molina)
================================================================
Unidades: MANACAPURU, AÇÃO MANACAPURU.
...
mais robusto. Ex.: "Perícia ADM (MANACAPURU)".

Colocar este arquivo em /pages, ao lado de painel_template_padrao.html.
...
# ===== PAINEL PADRÃO POR UNIDADE =====
# set_page_config precisa ser o 1º comando Streamlit; só depois lemos a URL.
st.set_page_config(page_title="Painel V360 · TV", layout="wide", initial_sidebar_state="collapsed")
_qp = st.query_params
UNIDADE_PARAM = (_qp.get("unidade") or "MANACAPURU").strip().upper()
...
MINUTOS_SYNC = 5
ALTURA_TV    = 1040
NUCLEO_LABEL = _qp.get("nucleo") or UNIDADE_PARAM.title()  # núcleo exibido nas telas
...
UNIDADES = [UNIDADE_PARAM, f"AÇÃO {UNIDADE_PARAM}"]

# Nome curto para caber na TV
UNI_CURTO = {u: u for u in UNIDADES}
...
# ---------------------------------------------------------------------
UNIDADES_PEND = list(UNIDADES)
UNIDADES_META = [UNIDADE_PARAM]
...
# PÁGINA
# ======================================================================
# (set_page_config já foi chamado no topo, com a unidade da URL)
st_autorefresh(interval=MINUTOS_SYNC * 60 * 1000, key=f"tv_padrao_{UNIDADE_PARAM}")
st.markdown("""
<style>
...
def _classifica(subtipo):
    """(unidade, categoria) a partir do nome do subtipo. Ex.:
       'Perícia ADM (MANACAPURU)' -> ('MANACAPURU', 'pericia')."""
...
template = Path(__file__).with_name("painel_template_padrao.html").read_text(encoding="utf-8")
