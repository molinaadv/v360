...
SLUG         = "aparecida"
SEG_POR_TELA = [30000, 30000, 30000]     # ms por tela: [Tela 1, Tela 2, Tela 3]
...
<style>
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  .kcard{background:linear-gradient(180deg,var(--panel) 0%,var(--panel2) 100%);
    border:1px solid var(--line);border-radius:18px;padding:18px 20px;display:flex;flex-direction:column;gap:12px}
  ...
  .kduo{display:grid;grid-template-columns:1fr 1fr;gap:12px}
...
<body>
  ...
  <footer>Alvará &amp; Levantamento · Aparecida · {{RODAPE}}</footer>
</body>
</html>
"""
...
# ───────── TV ÚNICA (só Tela 3 · Alvará) ─────────
def _esc(h):
    return (h.replace("\\","\\\\").replace("`","\\`").replace("${","\\${").replace("</script","<\\/script"))

try:
    h3=_esc(render_t3())
    solo=("<!DOCTYPE html><html><head><meta charset='UTF-8'><style>"
      ...
      "</style></head><body><iframe id='tv'></iframe><script>"
      "document.getElementById('tv').srcdoc=`"+h3+"`;"
      "</script></body></html>")
    st.components.v1.html(solo, height=1040, scrolling=False)
except Exception as e:
    st.error(f"Falha ao carregar o painel: {e}")
