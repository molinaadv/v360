...
# Tela 4 — Pendências por Unidade (2 gráficos: Atendimento Novo + Organização da Pasta)
COL_UNIDADE="unidade_nome"
A_ATENDIMENTO_NOVO="Atendimento Novo"
A_ORGANIZACAO_PASTA="Organização da Pasta"
ASSUNTOS_T4=[A_ATENDIMENTO_NOVO,A_ORGANIZACAO_PASTA]
...
<body>
  <header>
    <div class="htitle">
      ...
      <div>
        ...
        <div class="sub">Painel Gerência · fluxo da inicial: confecção → revisão → protocolo</div>
      </div>
    </div>
    <div class="meta">
  ...
  <footer>Painel Gerência · Tela 1 de 6 · {{RODAPE}}</footer>
</body>
</html>
"""
...
<body>
  <header>
    <div class="htitle">
      ...
      <div>
        ...
        <div class="sub">Painel Gerência · desfechos · em aberto + cumpridos no mês</div>
      </div>
    </div>
    <div class="meta">
  ...
  <footer>Painel Gerência · Tela 2 de 6 · {{RODAPE}}</footer>
</body>
</html>
"""
...
<body>
  <header>
    <div class="htitle">
      ...
      <div>
        ...
        <div class="sub">Painel Gerência · pagamento ao cliente · em aberto + cumpridos no mês</div>
      </div>
    </div>
    <div class="meta">
      <span class="legend"><span><i style="background:var(--warn)"></i>Em aberto</span><span><i style="background:var(--ok)"></i>Cumprido no mês</span></span>
  ...
  <footer>Painel Gerência · Tela 3 de 6 · {{RODAPE}}</footer>
</body>
</html>
"""
...
TEMPLATE_T4 = r"""<!DOCTYPE html>
<html lang="pt-BR">
<head>
<meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>Tela 4 — Pendências por Unidade</title>
<style>
  :root{--bg:#0b1220;--panel:#141d2e;--panel2:#1b2740;--line:#26324d;--ink:#f2f6ff;--muted:#93a1bd;--dim:#6b7a99;
    --accent:#5b8cff;--warn:#f5a524;--ok:#2fce8f;--azul:#4fb0e8;}
  *{box-sizing:border-box;margin:0;padding:0}
  html,body{height:100%}
  body{font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,Inter,Arial,sans-serif;
    background:radial-gradient(1300px 800px at 12% -12%,#17253e 0%,var(--bg) 55%);
    color:var(--ink);min-height:100%;padding:22px clamp(16px,3vw,46px) 18px;display:flex;flex-direction:column;gap:14px}
  header{display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:12px}
  .htitle{display:flex;align-items:center;gap:15px}
  .badge-num{width:46px;height:46px;border-radius:13px;background:linear-gradient(135deg,var(--azul),#2b7fb8);
    display:grid;place-items:center;font-weight:800;font-size:21px;color:#04141f;box-shadow:0 10px 26px -10px var(--azul)}
  h1{font-size:clamp(19px,2.3vw,29px);font-weight:800;letter-spacing:-.02em;line-height:1.05}
  .htitle .sub{color:var(--muted);font-size:13.5px;margin-top:3px}
  .meta{display:flex;align-items:center;gap:9px;flex-wrap:wrap}
  .pill{font-size:12px;font-weight:700;padding:7px 13px;border-radius:999px;border:1px solid var(--line);
    background:var(--panel2);color:var(--muted);display:inline-flex;align-items:center;gap:7px}
  .pill .live{width:8px;height:8px;border-radius:50%;background:var(--ok);box-shadow:0 0 0 4px rgba(47,206,143,.2)}
  .pill.scope{background:rgba(91,140,255,.12);border-color:rgba(91,140,255,.4);color:#b7c9ff}
  .charts{flex:1;min-height:0;display:flex;flex-direction:column;gap:14px}
  .chart{flex:1;min-height:0;background:linear-gradient(180deg,var(--panel) 0%,var(--panel2) 100%);
    border:1px solid var(--line);border-radius:16px;padding:12px 18px 6px;display:flex;flex-direction:column;gap:2px}
  .chead{display:flex;align-items:baseline;justify-content:space-between;gap:12px}
  .ct{font-size:14px;font-weight:800;color:var(--ink)}
  .ctot{font-size:12px;font-weight:800;color:var(--warn);letter-spacing:.04em}
  .plot{flex:1;min-height:0;display:flex;align-items:flex-end;gap:4px;padding:14px 4px 62px;position:relative}
  .bcol{flex:1 1 0;min-width:0;height:100%;display:flex;flex-direction:column;justify-content:flex-end;align-items:center;position:relative}
  .bval{font-size:10px;font-weight:700;color:var(--muted);margin-bottom:3px;line-height:1}
  .bfill{width:min(72%,28px);border-radius:4px 4px 0 0;background:linear-gradient(180deg,#f7b84e,#f2990f);min-height:2px}
  .blab{position:absolute;top:calc(100% + 7px);left:50%;font-size:9px;color:var(--dim);white-space:nowrap;
    transform:translateX(-100%) rotate(-45deg);transform-origin:right top;pointer-events:none}
  .empty{color:var(--dim);font-size:14px;margin:auto}
  .foot{display:grid;grid-template-columns:1fr 1fr;gap:14px}
  .fcard{background:var(--panel2);border:1px solid var(--line);border-radius:13px;padding:11px 16px;
    display:flex;align-items:center;justify-content:space-between;gap:12px}
  .fname{font-size:13px;font-weight:800;color:var(--ink)}
  .fnums{display:flex;align-items:baseline;gap:16px}
  .fnums .grp{display:flex;align-items:baseline;gap:7px}
  .fnums b{font-size:24px;font-weight:800;line-height:1}
  .fnums b.a{color:var(--warn)} .fnums b.c{color:var(--ok)}
  .fnums small{font-size:11px;color:var(--muted);font-weight:600}
</style>
</head>
<body>
  <header>
    <div class="htitle">
      ...
      <div><h1>Pendências por Unidade</h1>
      <div class="sub">Painel Gerência · em aberto por unidade · ordem decrescente</div></div>
    </div>
    <div class="meta">
      <span class="pill scope">● todas as unidades</span>
      <span class="pill">Competência · {{COMPETENCIA}}</span>
  ...
  <div class="charts">
    <div class="chart">
      <div class="chead"><span class="ct">1. Atendimento Novo — em aberto</span><span class="ctot">TOTAL {{AN_TOTAL}}</span></div>
      <div class="plot">{{AN_BARS}}</div>
    </div>
    <div class="chart">
      <div class="chead"><span class="ct">2. Organização da Pasta — em aberto</span><span class="ctot">TOTAL {{OP_TOTAL}}</span></div>
      <div class="plot">{{OP_BARS}}</div>
    </div>
  </div>
  <div class="foot">
    <div class="fcard"><span class="fname">Atendimento Novo</span>
      <div class="fnums"><span class="grp"><b class="a">{{AN_AB}}</b><small>em aberto</small></span>
        <span class="grp"><b class="c">{{AN_CU}}</b><small>cumpridos / mês</small></span></div></div>
    <div class="fcard"><span class="fname">Organização da Pasta</span>
      <div class="fnums"><span class="grp"><b class="a">{{OP_AB}}</b><small>em aberto</small></span>
        <span class="grp"><b class="c">{{OP_CU}}</b><small>cumpridos / mês</small></span></div></div>
  </div>
  <footer style="color:var(--dim);font-size:12px;text-align:center">Painel Gerência · Tela 4 de 6 · {{RODAPE}}</footer>
</body>
</html>
"""

def _uni_desc(dfa):
    s=dfa[COL_UNIDADE].fillna("— sem unidade").astype(str).str.strip().replace("","— sem unidade")
    return [{"nome":str(k),"n":int(v)} for k,v in s.value_counts().items()]

@st.cache_data(ttl=CACHE_TTL, show_spinner=False)
def carregar_t4():
    competencia, ym=_mes_atual()
    df=_pull(ASSUNTOS_T4)
    charts={}
    if df.empty or COL_UNIDADE not in df.columns:
        for k in ["an","op"]: charts[k]={"unidades":[],"aberto":0,"cumprido":0}
        return {"competencia":competencia,"fonte":"ao vivo (Supabase)","charts":charts}
    stt=df[COL_STATUS].fillna("")
    aberto=df[stt.isin(STATUS_ABERTO)]; cumpr=df[(stt==STATUS_CUMPRIDO)&(df[COL_MES_CONCL].astype(str)==ym)]
    for k,sub in [("an",A_ATENDIMENTO_NOVO),("op",A_ORGANIZACAO_PASTA)]:
        ab=aberto[aberto[COL_ASSUNTO]==sub]
        charts[k]={"unidades":_uni_desc(ab),"aberto":int(len(ab)),"cumprido":int((cumpr[COL_ASSUNTO]==sub).sum())}
    return {"competencia":competencia,"fonte":"ao vivo (Supabase)","charts":charts}

def _barras_v(U):
    if not U: return '<div class="empty">Sem tarefas em aberto.</div>'
    mx=max((u["n"] for u in U), default=1) or 1
    out=""
    for u in U:
        nm=u["nome"]; nm=(nm[:15]+"…") if len(nm)>16 else nm
        h=round(u["n"]/mx*100)
        out+=f'<div class="bcol"><div class="bval">{u["n"]}</div><div class="bfill" style="height:{h}%"></div><div class="blab">{nm}</div></div>'
    return out

def render_t4():
    d=carregar_t4(); an=d["charts"]["an"]; op=d["charts"]["op"]
    tpl=TEMPLATE_T4
    repl={"{{COMPETENCIA}}":d["competencia"],"{{STATUS_FONTE}}":d["fonte"],
      "{{AN_TOTAL}}":str(an["aberto"]),"{{AN_BARS}}":_barras_v(an["unidades"]),
      "{{AN_AB}}":str(an["aberto"]),"{{AN_CU}}":str(an["cumprido"]),
      "{{OP_TOTAL}}":str(op["aberto"]),"{{OP_BARS}}":_barras_v(op["unidades"]),
      "{{OP_AB}}":str(op["aberto"]),"{{OP_CU}}":str(op["cumprido"]),
      "{{RODAPE}}":f"atualizado {datetime.now():%d/%m %H:%M}"}
    for k,v in repl.items(): tpl=tpl.replace(k,v)
    return tpl
    ...
    for i, u in enumerate(unidades, 1):
        ...
        <div class="ur">
          ...
          <div>
            ...
            <div class="us">{participacao:.1f}% do total</div>
          </div>
...
try:
    ...
    combined=("<!DOCTYPE html><html><head><meta charset='UTF-8'><style>"
      ...
      "display:flex;align-items:center;gap:10px;z-index:9;padding:6px 12px;border-radius:999px;"
      "background:rgba(11,18,32,.55);backdrop-filter:blur(6px);border:1px solid rgba(38,50,77,.7)}"
      "#pbtn{width:26px;height:22px;display:flex;align-items:center;justify-content:center;cursor:pointer;"
      "color:#cdd8ee;font-size:13px;line-height:1;border-radius:6px;user-select:none}"
      "#pbtn:hover{background:rgba(255,255,255,.10);color:#fff}"
      ...
      "btn.addEventListener('click',toggle);nav.appendChild(btn);"
      ...
      "function paint(a){segs.forEach(function(b,k){b.style.transition='none';b.style.width=(k<a?'100%':'0');});}"
      "function anim(from,dur){var c=segs[i];c.style.transition='none';c.style.width=from;void c.offsetWidth;"
      "requestAnimationFrame(function(){c.style.transition='width '+dur+'ms linear';c.style.width='100%';});}"
      "function show(idx){clearTimeout(tmr);paused=false;btn.textContent='\u23f8';i=idx;f.srcdoc=TELAS[i];paint(i);"
      "remaining=DUR[i%DUR.length];startAt=Date.now();anim('0',remaining);"
      "tmr=setTimeout(function(){show((i+1)%TELAS.length);},remaining);}"
      "function pause(){if(paused)return;paused=true;btn.textContent='\u25b6';clearTimeout(tmr);"
      "remaining=Math.max(0,remaining-(Date.now()-startAt));var c=segs[i];var w=getComputedStyle(c).width;"
      "c.style.transition='none';c.style.width=w;}"
      "function resume(){if(!paused)return;paused=false;btn.textContent='\u23f8';startAt=Date.now();"
      "anim(getComputedStyle(segs[i]).width,remaining);"
      "tmr=setTimeout(function(){show((i+1)%TELAS.length);},remaining);}"
      "function toggle(){paused?resume():pause();}"
      "document.addEventListener('keydown',function(e){if(e.code==='Space'){e.preventDefault();toggle();}});"
      "show(0);"
      "</script></body></html>")
