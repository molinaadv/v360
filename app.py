...
# módulo opcional: o chat/assistente. Se falhar ao importar, só a aba
# "Assistente" avisa — o resto do app continua funcionando.
try:
    import pagina_assistente
    _ERRO_ASSISTENTE = None
except Exception as _e:
    pagina_assistente = None
    _ERRO_ASSISTENTE = _e
...
try:
    if pagina == "Executivo":
        ...
        if pagina_assistente is None:
            st.error("A página 'Assistente' não pôde ser carregada.")
            if _ERRO_ASSISTENTE is not None:
                st.exception(_ERRO_ASSISTENTE)
        else:
            permitidas = auth.unidades_permitidas()
